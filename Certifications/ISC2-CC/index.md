
# 🛡️ ISC2 Certified in Cybersecurity (CC) — ✅ COMPLETED

| Field | Details |
|-------|---------|
| Status | ✅ Certified |
| Date Earned | May 2026 |
| Verify Badge | [🔗 Credly/ISC2 Link] |

## 📋 Domain Documentation
Click any domain to view theory notes & practical activities:

| Domain | Title | Theory |
|--------|-------|--------|-----------|
| 01 | [Security Principles](./Domain-01/theory.md) | ✅ Completed | 
| 02 | [Incident Response](./Domain-02/theory.md) | ✅ Completed | 
| 03 | [Access Control Concepts](./Domain-03/theory.md) | ✅ Completed |
| 04 | [Network Security](./Domain-04/theory.md) | ✅ Completed | 
| 05 | [Security Operations](./Domain-05/theory.md) | ✅ Completed | 

## 🏆 Domain Completion Certificates
*Official (ISC)² PDF certificates for each completed domain.*

| Domain | Certificate | Download |
|--------|-------------|----------|
| 01 | Security Principles | [📄 View PDF](../../Assets/certificates/isc2/isc2-domain-01-security-principles.pdf) |
| 02 | Incident Response, BC & DR | [📄 View PDF](../../Assets/certificates/isc2/isc2-domain-02-incident-response.pdf) |
| 03 | Access Control Concepts | [📄 View PDF](../../Assets/certificates/isc2/isc2-domain-03-access-control.pdf) |
| 04 | Network Security | [📄 View PDF](../../Assets/certificates/isc2/isc2-domain-04-network-security.pdf) |
| 05 | Security Operations | [📄 View PDF](../../Assets/certificates/isc2/isc2-domain-05-security-operations.pdf) |

> ℹ️ *Certificates issued by (ISC)². Click "View PDF" to open the official document in your browser.*

## 🏆 Proof of Completion
![ISC2 CC Badge](../../Assets/badges/isc2-cc-badge.png)  
📄 [Download Certificate](../../Assets/certificates/isc2-cc-certificate.pdf)

---
*🔙 * [Back to Certifications Hub](../index.md)*

