# 🛡 Microsoft Certified: Security Operations Analyst Associate

## Full Course: Defend against cyberthreats with Microsoft's security operations platform

| Field | Details |
|-------|---------|
| Status | 🟡 In Progress |
| Modules Completed | 3/10 |
| Verify Credential | [🔗 Microsoft Credential Link] |

---


## 📚 Learning Path Progress

| Path | Title | Theory | Trophy | Status |
|------|-------|--------|--------|--------|
| 01 | Mitigate threats using Microsoft Defender XDR | [📖 View](./Module-01/theory.md) | [🏆 View](./Module-01/trophy.md) | ✅ Completed |
| 02 | Mitigate threats using Microsoft Security Copilot | [📖 View](./Module-02/theory.md) | [🏆 View](./Module-02/trophy.md) | ✅ Completed |
| 03 | Mitigate threats using Microsoft Purview | [📖 View](./Module-03/theory.md) | [🏆 View](./Module-03/trophy.md) | ✅ Completed |
| 04 | Mitigate threats using Microsoft Defender for Endpoint | [📖 View](./Module-04/theory.md) | ⚪️ Pending | 🔄 In Progress |
| 05 | Mitigate threats using Microsoft Defender for Cloud | [📖 View](./Module-05/theory.md) | ⚪️ Pending | ⚪️ Not Started |
| 06 | Create queries for Microsoft Sentinel using Kusto Query Language (KQL) | [📖 View](./Module-06/theory.md) | ⚪️ Pending | ⚪️ Not Started |
| 07 | Configure your Microsoft Sentinel environment | [📖 View](./Module-07/theory.md) | ⚪️ Pending | ⚪️ Not Started |
| 08 | Connect logs to Microsoft Sentinel | [📖 View](./Module-08/theory.md) | ⚪️ Pending | ⚪️ Not Started |
| 09 | Create detections and perform investigations using Microsoft Sentinel | [📖 View](./Module-09/theory.md) | ⚪️ Pending | ⚪️ Not Started |
| 10 | Perform threat hunting in Microsoft Sentinel | [📖 View](./Module-10/theory.md) | ⚪️ Pending | ⚪️ Not Started |

---

## 🛠️ Hands-On Labs & Practical Work

> *Documentation of integrated labs and sandbox exercises completed within the theory modules.*

| Lab | Topic | Related Module | Documentation | Status |
|-----|-------|----------------|---------------|--------|
| Lab 01 | [Lab Title] | Module 01 | [📄 View](./labs/lab-01.md) | 🔄 In Progress |
| Lab 02 | [Lab Title] | Module 01 | [📄 View](./labs/lab-02.md) | ⚪️ Not Started |


---
> 💡 Note: Links are placeholders. I'll activate them as I complete each course.

---

*🔙 [Back to Certifications Hub](../index.md)*