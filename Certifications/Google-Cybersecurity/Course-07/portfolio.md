...
After purging the unauthorized IPs from the list in memory, I need to save the updated allow list back to the text file. First, I convert the `ip_addresses` list back into a single string using the `.join()` method. I apply `.join()` to the character `" "` in order to separate the elements in the file by placing them on a new line. Next, I use a second `with` statement to open the `import_file` again, this time in write mode ("w"). Inside this block, I use the `.write()` method to overwrite the file's old contents with the newly formatted, sanitized string.
